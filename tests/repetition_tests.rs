...
mod tests {
    ...
    fn test_threefold_repetition() {
        ...
        let repeating_moves = [
            "d1e2", "d8e7", "e2d1", "e7d8", "d1e2", "d8e7", "e2d1", "e7d8",
        ];
        ...
        assert!(
            ...
            "Should be a draw by repetition"
        );
    }
    ...
    fn test_repetition_with_different_castling_rights() {
        ...
        let moves = [
            "e2e4", "e7e5", "g1f3", "b8c6", "f1b5", "a7a6", "b5a4", "g8f6", "e1g1", "b7b5", "a4b3",
            "d7d6", "d1e2", "d8e7", "e2d1", "e7d8", // Second time position appears
            "d1e2", "h8g8", // Black loses kingside castle rights
            "e2d1",
            "g8h8", // Third time position appears, but castle rights have changed so it's not a draw
            "d1e2", "d8e7", "e2d1",
            "e7d8", // Fourth time position appears (second with these castling rights)
            "d1e2", "a8b8", // Black loses queenside castle rights
            "e2d1",
            "b8a8", // Fifth time position appears, but castle rights have changed so it's not a draw
            "d1e2", "d8e7", "e2d1",
            "e7d8", // Sixth time position appears (second with these castling rights)
        ];
        ...
        assert!(
            !board.is_draw_by_repetition(),
            "Should not be a draw due to different castling rights"
        );

        // Finally repeat the position again
        let repeating_moves = ["d1e2", "d8e7", "e2d1", "e7d8"];
    ...
    fn test_repetition_with_different_en_passant() {
        ...
        let moves = [
            "e2e4", "e7e6", "e4e5", "d7d5", // En passant move available
            "d1e2", /* En passant rights lost */ "d8e7", "e2d1",
            "e7d8", // Second time position appears, but en passant has changed
            "d1e2", "d8e7", "e2d1",
            "e7d8", // Third time position appears, but en passant has changed so it's not a draw
        ];
        ...
        assert!(
            !board.is_draw_by_repetition(),
            "Should not be a draw due to different en passant"
        );
